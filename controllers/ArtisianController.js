...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, uniqueSuffix + path.extname(file.originalname)); // Save with unique name and correct extension
  }
});
...
const adduserCart = async (req, res) => {
  const { userId, productIds, name } = req.body; // productIds as [{ productId: 'id1', quantity: 2 }, ...]
